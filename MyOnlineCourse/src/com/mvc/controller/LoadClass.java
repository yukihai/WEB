...
public class LoadClass extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String text="";
		text+="<table border = 2><tr><th>Mã lớp học</th><th>Tên lớp học</th><th>Tên giáo viên</th><th></th></tr>";
		int i=0;
		while(result[i][0]!=null)
		{
			text+="<tr><td>"+result[i][0]+"</td><td>"+result[i][1]+"</td><td>"+result[i][2]+
					"</td><td><button class=\"btn btn-warning\" id=\"bt"+result[i][0]+"\" value=\""+result[i][0]+ "\" onclick=\"ToClass('"+result[i][0]+"')\">Xem</button></td></tr>";
			i++;
		}
		text+="</table>";

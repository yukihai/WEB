...
public class LoadList extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		String text = "";
		text += "<table border = 2><tr><th>TestID</th><th>Title</th><th>CourseName</th><th>Quantity</th><th>Time</th><th>Author</th><th></th></tr>";
		int i = 0;
		while (result[i][0] != null) {
			text += "<tr><td>" + result[i][0] + "</td><td>" + result[i][1] + "</td><td>" + result[i][2] + "</td><td>"
					+ result[i][3] + "</td><td>" + result[i][4] + "</td><td>" + result[i][5]
					+ "</td><td class=\"mybt\"><button id=\"bt" + result[i][0] + "\" value=\"" + result[i][0]
					+ "\" onclick=\"DoTest('" + result[i][0] + "','" + result[i][3] + "')\">Làm bài</button></td></tr>";
			i++;
		}
		text += "</table>";
